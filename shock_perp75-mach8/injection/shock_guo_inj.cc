...
begin_initialization {
  ...
  double wpe_wce = 0.5774;
  double gam = 1.02;       // Gamma of plasma flow
  double V1 = 0.15;       // speed of the flow
  double Te = 0.168;       // Electron temperature in units of me*c**2 in plasma rest frame
  double Ti = 0.168;       // Ion temperature in units of me*c**2 in plasma rest frame
  ...
  double theta = 63.0;     //  Shock angle

  double pi = 3.1415927;
  double cs = cos(theta/180.0*pi);
  double sn = sin(theta/180.0*pi);

  double taui    = 100.0; // simulation wci's to run
  ...
  double Lz  = 40*de;      // size of box in z dimension
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                 \
 ...
 // Set the boundary at x=Lx to be absorbing for particles and fields
  ...
  double nparticles_per_proc = 6*Ne/nproc();
...
//#define BZWAVE DBZ_WAVE(0.025,2,1,0.5) + DBZ_WAVE(0.02,3,2,-0.2) + DBZ_WAVE(0.045,4,3,-0.3) + DBZ_WAVE(0.04,5,4,0.3) + DBZ_WAVE(0.05,6,5,0.8) + DBZ_WAVE(0.075,7,6,0.8)
  ...
  set_region_field( everywhere, 0, -V1*sn*b0, 0, cs*b0, 0.0, sn*b0);
...
#define yz_EDGE_LOOP(x) XYZ_LOOP(x,x,1,ny+1,1,1+nz)
  ...
  // Right Boundary
  if (global->right) {
    yz_EDGE_LOOP(nx+1) field(x,y,z).ey = -Vflow*b0*sn;
    yz_EDGE_LOOP(nx+1) field(x,y,z).cbz = b0*sn; 
  }
