...
begin_globals {
    ...
    DumpParameters hHdParams;     // Proton
    DumpParameters hAdParams;     // Alpha particle
    DumpParameters hTdParams;     // Helium-3
    std::vector<DumpParameters *> outputParams;

    // Variables for the energy diagnostics
    edata ede;                    // parameters for electron species
    edata edH;                    // parameters for ion species
    edata eda;                    // parameters for alpha particles
    edata edt;                    // parameters for Helium-3
...
begin_initialization {
  ...
  int particle_select = 4000; // track one every particle_select particles
  ...
  if (hydro_at_tracer) {
    ...
    double bg = 1e-5;          // guide field
    double theta   = 0.0;     // B0 = Bx
    double taui    = 50;     // simulation wci's to run
    double alpha_ab = 0.1;   // Alpha particle abundance
    double he3_ab = 1.0;     // 3He/4He
    double eb_ab = 0.1;     // 3He/4He
    ...
    double ma = mi * 4;                      // Alpha particle mass
    double mt = mi * 3;                      // Helium-3 particle mass
    double vthi = vthe*sqrt(Ti_Te/mi_me);    // Ion thermal velocity
    double vtha = vthi * 0.5;                // Alpha particle thermal velocity
    double vtht = vthi * sqrt(1.0/3);        // Helium-3 particle thermal velocity
    ...
    double Lx  = 25.0*di; // size of box in x dimension
    double Ly  = 1.0/sqrt(mi_me)*di;     // size of box in y dimension
    double Lz  = 12.5*di; // size of box in z dimension
    ...
    double Na  = Ne * alpha_ab;  // total macro alpha particles
    double Nt  = Na * he3_ab;    // total macro Helium-3 particles
    ...
    double weight = Np/Ne;
    Na  = trunc_granular(Na,nproc()); // Make it divisible by number of processors
    Nt  = trunc_granular(Nt,nproc()); // Make it divisible by number of processors
    double qe = -ec*weight;  // Charge per macro electron
    double qi =  ec*weight;  // Charge per macro ion
    double qa =  qi*2;      // Charge per macro alpha particle
    double qt =  qi*2;      // Charge per macro alpha particle
    double Ni = Ne;         // total number of macro ions
    Ne += Na * 2;           // the alpha particles have charge 2
    Ne += Nt * 2;           // the Helium-3 particles have charge 2
    //double Nebeam = 0.1*Ne;
    //double Neback = 0.9*Ne;
    //double Vbeam = 0.1*c;
  ...
  Ntracer = trunc_granular(Ntracer, nproc());
    ...
    int restart_interval = int(50000.0/(wci*dt));
    //int restart_interval = 2000;
    int energies_interval = 100;
  int tracer_interval = 1;
  //int tracer_interval = int(5.0/(wpe*dt));
  ...
  int spectrum_interval  = interval;
    ...
    int eparticle_interval = 10000*interval;
    int Hparticle_interval = 10000*interval;
  ...
  double emax_spect = 1.0E8;    // maximum energy for energy spectrum diagnostics
  double emin_spect = 1.0E-2;   // minimum energy for energy spectrum diagnostics
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                   \
  ...
  double nparticles_per_proc_alpha = 5*Na/nproc();
  double nparticles_per_proc_He3 = 5*Nt/nproc();
  double nmovers_per_proc    = 0.2*nparticles_per_proc;
  double nmovers_per_proc_alpha    = 0.2*nparticles_per_proc_alpha;
  double nmovers_per_proc_He3    = 0.2*nparticles_per_proc_He3;
  ...
  sim_log( "num_tracer_particles_per_proc = "<<ntracers_per_proc );
    ...
    species_t *alpha = define_species("alpha",  2*ec, ma, nparticles_per_proc,
          nmovers_per_proc, ion_sort_interval, sort_method);
    species_t *he3 = define_species("he3",  2*ec, mt, nparticles_per_proc,
          nmovers_per_proc, ion_sort_interval, sort_method);
    species_t *s, *s0;
    global->nsp = 2;
    global->nsp_efield = 0;
  ...
  hijack_tracers(2);
    ...
    } // Finished dumping simulation information
...
# define VDY -(b0/L)/(cosh(z/L)*cosh(z/L))
...
# define DBZ dbz*cos(pi*z/Lz)*sin(2.0*pi*(x-0.5*Lx)/Lpert)
    ...
    set_region_field( everywhere, 0, 0, 0, (BX+DBX)*cs+BY*sn,
            -(BX+DBX)*sn+BY*cs, DBZ);
  ...
  int iparticle  = 0;   // particle index
    ...
    } // Finished injecting protons

    // Inject Alpha particles
    repeat ( Na/nproc() ) {
        double x, y, z, ux, uy, uz, upa1, upe1, uz1, gu1;
        // Alpha particles are spatially uniform Maxwellian with no drifts

        x = uniform(rng(0),xmin,xmax);
        y = uniform(rng(0),ymin,ymax);
        z = uniform(rng(0),zmin,zmax);

        upa1 = normal(rng(0),0,vtha);
        upe1 = normal(rng(0),0,vtha);
        uz1 = normal(rng(0),0,vtha);
        gu1 = sqrt(1.0+upa1*upa1+upe1*upe1+uz1*uz1);
        ux = upa1;
        uy = upe1;
        uz = uz1;

        inject_particle(alpha, x, y, z, ux*cs+uy*sn,-ux*sn+uy*cs, uz, weight, 0, 0 );
    }

    // Inject Helium-3 particles
    repeat ( Nt/nproc() ) {
        double x, y, z, ux, uy, uz, upa1, upe1, uz1, gu1;
        // Alpha particles are spatially uniform Maxwellian with no drifts
        ...
        upa1 = normal(rng(0),0,vtht);
        upe1 = normal(rng(0),0,vtht);
        uz1 = normal(rng(0),0,vtht);
        gu1 = sqrt(1.0+upa1*upa1+upe1*upe1+uz1*uz1);
        ux = upa1;
        uy = upe1;
        uz = uz1;

        inject_particle(he3, x, y, z, ux*cs+uy*sn,-ux*sn+uy*cs, uz, weight, 0, 0 );
    }
    ...
    global->hAdParams.format = band;
    global->hTdParams.format = band;
...
#ifdef DUMP_WITH_HDF5
  sprintf(global->hAdParams.baseDir, "hydro_hdf5");
  dump_mkdir(global->hAdParams.baseDir);
#else
  sprintf(global->hAdParams.baseDir, "hydro/%d",NUMFOLD);
  dump_mkdir(global->hAdParams.baseDir);
#endif

#ifdef DUMP_WITH_HDF5
  sprintf(global->hTdParams.baseDir, "hydro_hdf5");
  dump_mkdir(global->hTdParams.baseDir);
#else
  sprintf(global->hTdParams.baseDir, "hydro/%d",NUMFOLD);
  dump_mkdir(global->hTdParams.baseDir);
#endif

    //sprintf(global->hHdParams.baseDir, "hydro");
    //sprintf(global->hAdParams.baseDir, "hydro");
    //sprintf(global->hTdParams.baseDir, "hydro");

    // base file name for fields output
    sprintf(global->hHdParams.baseFileName, "Hhydro");
    sprintf(global->hAdParams.baseFileName, "Ahydro");
    sprintf(global->hTdParams.baseFileName, "Thydro");

    global->hHdParams.stride_x = 1;
    global->hHdParams.stride_y = 1;
    global->hHdParams.stride_z = 1;

    global->hAdParams.stride_x = 1;
    global->hAdParams.stride_y = 1;
    global->hAdParams.stride_z = 1;

    global->hTdParams.stride_x = 1;
    global->hTdParams.stride_y = 1;
    global->hTdParams.stride_z = 1;
    ...
    // add ion and alpha particles species parameters to list
    global->outputParams.push_back(&global->hHdParams);
    global->outputParams.push_back(&global->hAdParams);
    global->outputParams.push_back(&global->hTdParams);
    ...
    */
    ...
    global->hAdParams.output_variables( all );
    global->hTdParams.output_variables( all );
    ...
    create_hydro_list(varlist, global->hAdParams);

    sim_log ( "Alpha particles variable list: " << varlist );

    create_hydro_list(varlist, global->hTdParams);

    sim_log ( "Helium-3 particles variable list: " << varlist );
    ...
    global->eda.sp_id = alpha->id;
    global->eda.vth = sqrt(2.0)*vtha;
    sprintf(global->eda.fname, global->hAdParams.baseFileName);

    global->edt.sp_id = he3->id;
    global->edt.vth = sqrt(2.0)*vtht;
    sprintf(global->edt.fname, global->hTdParams.baseFileName);
...
#include "viou/dump_field_with_hdf5.cc"
  ...
  if(step() == 1 || should_dump(fields)) dump_fields_hdf5(global->fdParams.baseDir, 0);
  ...
  hydro_dump_flag.resetToDefaults();
    ...
    if(should_dump(Hhydro)) hydro_dump("alpha", global->hAdParams);

    if(should_dump(Hhydro)) hydro_dump("he3", global->hTdParams);
  ...
  if ( step() == num_step ) {
    ...
    // Dump particle data
    char subdir[36];
    if ( should_dump(eparticle) && step() > 0*(global->fields_interval)) {
        ...
        sprintf(subdir,"particle/T.%d/aparticle",step());
        dump_particles("alpha",subdir);
        sprintf(subdir,"particle/T.%d/tparticle",step());
        dump_particles("he3",subdir);
    }
