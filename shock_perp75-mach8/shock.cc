...
begin_initialization {
  ...
  double V1 = 0.16;       // speed of the flow
  double Te = 0.01;       // Electron temperature in units of me*c**2 in plasma rest frame
  double Ti = 0.01;       // Ion temperature in units of me*c**2 in plasma rest frame
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                 \
 ...
 // Set the boundary at x=Lx to be absorbing for particles and fields
  ...
  double nparticles_per_proc = 16*Ne/nproc();
...
#define yz_EDGE_LOOP(x) XYZ_LOOP(x,x,1,ny+1,1,1+nz)
  ...
  if (global->right) {
    ...
    yz_EDGE_LOOP(nx+1) field(x,y,z).cbx = b0*cs;
  }
